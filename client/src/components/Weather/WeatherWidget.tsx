...
export default function WeatherWidget({ lat, lng, date, compact = false, stacked = false, locationName = null }: WeatherWidgetProps) {
  ...
  if (loading) {
    return (
      <span style={{ fontSize: 'calc(14px * var(--fs-scale-caption, 1))', color: '#d1d5db', ...fontStyle }}>…</span>
    )
  }

  if (failed || !weather) {
    return (
      <span style={{ fontSize: 'calc(14px * var(--fs-scale-caption, 1))', color: '#9ca3af', ...fontStyle }}>—</span>
    )
  }
